    ...
    try:
        ...
        def resolverCapcha():
            ...
            try:
                ...
                print("Capcha resuelta: {}".format(jsonParse))
                ...
                wait.until(EC.presence_of_element_located((By.ID, 'ctl00_cplPrincipal_CaptchaContinue'))).click()
                time.sleep(5)
                ...
                try:
                    ...
                    espero.until(EC.presence_of_element_located((By.NAME, 'ctl00$cplPrincipal$txtCaptcha')))
                    time.sleep(5)
                    ...
                    # input("finally 1")
                    espero = WebDriverWait(driver, 2)
                    tabla = espero.until(EC.presence_of_element_located((By.ID, 'ctl00_cplPrincipal_divRecord')))
                    print("Leyendo tabla...")
        ...
        def leearTablacontribuyente(contribuyente, paginas):
            ...
            def leearTablaBeauti(html):
                ...
                table = soup.find('table', attrs={'id': 'ctl00_cplPrincipal_grdRecord'})
                # arcaDj = converToArcadj(driver.get_screenshot_as_base64())
                table_body = table.find('tbody')
                print(table_body)
                # f = open('tabla.html', 'aw')
                # f.write(table_body)
                # f.close
                print("Leyendo tabla principal")
                # input("Leyendo tabla principal")

                # try:
                #     # print("ENTRPA TRUYYYYYYYYYYYYYYYYYYYYYY")
                #     paginacion = table_body.find_all('tr', attrs={'class': 'grillaRows'})
                #     print("PAGIANSASASAAS {}".format(paginacion))
#                 #     input("Esperando PAGIANSASASAAS")
...
#                 #         input("Esperando Cantidad")
                ...
                def RecolectarDatos(html):
                    print("RecolectarDatos")
                    # input("RecolectarDatos")
                    ...
                    for row in rows:
                        cols = row.find_all('td')
                        cols = [ele.text.strip() for ele in cols]
                        l2 = ["documentoCodigoPago", "reglamento", "falta", "fechaInfracEmision", "placa", "importe", "TipoDeFalta", "licenciaConducir", "estado"]
                        diccionari = dict(zip(l2, [ele for ele in cols]))
                        data.append(diccionari)
                        # print(item for item in data if len(item) > 0)
                    return [item for item in data if len(item) > 0]

                paginasTotales = 1
                print("PAGINAS POR RECORRER {}".format(paginasTotales))
                # input("Se imprimio PAGINAS POR RECORRER")
                if paginasTotales > 0:
            ...
            try:
                ...
                if 0 == 0:
                    ...
                    # arcaDj = converToArcadj(driver.get_screenshot_as_base64())
                    wait.until(EC.presence_of_element_located((By.ID, 'ctl00_cplPrincipal_divRecord')))
                    print("existe la tabla")
                    nombres = driver.find_element_by_id('ctl00_cplPrincipal_lblConductor').text
                    print("Procesando el contribuyente {}".format(nombres))
                    # arcaDj = converToArcadj(driver.get_screenshot_as_base64())
...
#                         }
                ...
                try:
                    ...
                    # sinTablas = driver.find_element_by_id('ctl00_cplPrincipal_lblMensajeSinDatos').text
                    # print("Respuesta de la lectura de la table {}".format(sinTablas))
                    wait.until(EC.presence_of_element_located((By.ID, 'ctl00_cplPrincipal_grdRecord')))
                    # nombres = driver.find_element_by_id('ctl00_cplPrincipal_lblAdministrado').text
                    # print("Procesando el contribuyente {}".format(nombres))
                    arcaDj = converToArcadj(driver.get_screenshot_as_base64())
                    # arcaDj = converToArcadj(driver.get_screenshot_as_base64())
...
#                         # driver.save_screenshot(path + "prueba.png")
        ...
        # Seleccionando el Iframe donde scrapear
        driver.switch_to.frame(driver.find_element_by_name("fraRightFrame"))
        wait.until(EC.presence_of_element_located((By.ID, 'ui-accordion-menu-header-2'))).click()
        ...
        try:
            ...
            wait.until(EC.presence_of_element_located((By.ID, 'menuOption08'))).click()
            time.sleep(1)
            # Selecionando busqueda por dni
        ...
        wait.until(EC.presence_of_element_located((By.NAME, 'ctl00$cplPrincipal$txtDNI'))).clear()
        # setear la dni
        wait.until(EC.presence_of_element_located((By.NAME, 'ctl00$cplPrincipal$txtDNI'))).send_keys(str(dni))
