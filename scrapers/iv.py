...
def impuestoVehicular(placa, webdrivertipo):
    try:
...
#                         # driver.save_screenshot(path + "prueba.png")
        ...
        wait.until(EC.presence_of_element_located((By.XPATH, '/html/body/form/div[2]/section/div/div/div[2]/div[1]/div[2]/div[1]/div[3]/a/img'))).click()
        time.sleep(5)
        """
        ...
        """
        try:
            wait.until(EC.presence_of_element_located((By.ID, 'tipoBusqueda'))).click()
            time.sleep(2)
            wait.until(EC.presence_of_element_located((By.ID, 'ctl00_cplPrincipal_optBuscaPlaca'))).click()
            time.sleep(2)
        except:
